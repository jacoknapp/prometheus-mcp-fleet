...
func (c *Spoke) LogValue() slog.Value {
	...
	return slog.GroupValue(
		slog.Any("hubEndpoints", c.HubEndpoints),
		slog.String("hubAPIURL", redactURL(c.HubAPIURL)),
		slog.String("hubCAFile", c.HubCAFile),
		slog.Bool("hubTLSInsecure", c.HubTLSInsecure),
		slog.Bool("allowInsecure", c.AllowInsecure),
		slog.String("enrollmentTokenFile", c.EnrollmentTokenFile),
		slog.String("clusterID", c.ClusterID),
		slog.String("clusterDisplayName", c.ClusterDisplayName),
		slog.String("clusterDescription", c.ClusterDescription),
		slog.String("clusterLabels", FormatClusterLabels(c.ClusterLabels)),
		slog.String("dataDir", c.DataDir),
		slog.String("prometheusURL", redactURL(c.PrometheusURL)),
		slog.Duration("prometheusTimeout", c.PrometheusTimeout),
		slog.String("prometheusBearerTokenFile", c.PrometheusBearerTokenFile),
		slog.String("prometheusTLSCAFile", c.PrometheusTLSCAFile),
		slog.Bool("prometheusTLSSkipVerify", c.PrometheusTLSSkipVerify),
		slog.Int64("prometheusMaxResponseBytes", c.PrometheusMaxResponseBytes),
		slog.Duration("factsRefreshInterval", c.FactsRefreshInterval),
		slog.Duration("reconnectMinBackoff", c.ReconnectMinBackoff),
		slog.Duration("reconnectMaxBackoff", c.ReconnectMaxBackoff),
