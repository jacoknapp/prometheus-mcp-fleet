...
| `--state-retention` | `720h` (30d) | How long an expired credential or a revocation for an already-expired certificate is kept before the hub prunes it, **on top of `--renew-grace`** (a revoked certificate can still reach `/renew` for that long after it expires, so the hub never prunes inside it). A forensics window, not a safety margin: nothing pruned can change an answer, even at `0`. |
