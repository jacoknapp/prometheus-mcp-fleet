...
real API server.
*/}}
{{- define "prometheus-mcp-hub.memoryBytes" -}}
{{- $q := . | toString | trim -}}
{{- $num := regexFind "^[0-9]+(\\.[0-9]+)?" $q -}}
{{- if eq $num "" -}}
{{- fail (printf "prometheus-mcp-hub: cannot read %q as a memory quantity." $q) -}}
{{- end -}}
...
{{- fail (printf "prometheus-mcp-hub: memory quantity %q has an unsupported suffix %q. Use bytes or one of k, M, G, T, P, E, Ki, Mi, Gi, Ti, Pi, Ei." $q $suffix) -}}
{{- end -}}
